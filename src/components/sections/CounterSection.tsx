...
export function CounterSection({ bg, counters }: Props) {
    ...
    >
      ...
      <Container className="relative z-10">
        <header className="mb-14 text-center md:mb-16">
          <p className="mb-2 text-xs font-semibold uppercase tracking-[0.28em] text-brand-primary">
            {t("sub")}
          </p>
          <h2 className="mb-4 font-display text-3xl font-semibold text-white md:text-4xl">
            {t("title")}
          </h2>
          <p className="mx-auto mb-8 max-w-2xl text-base leading-relaxed text-white/75 md:text-lg">
            {t("lead")}
          </p>
          <Link
            href="/menu"
        ...
        <div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-4">
          {counters.map((c, i) => (
            <Stat
              ...
              value={c.value}
              suffix={c.suffix}
              label={t(c.labelKey as "branches")}
              delayMs={i * 100}
            />
          ))}
        </div>
