...
import { saveBestSellersSectionHeadings } from "@/app/actions/best-sellers-section-admin";
import { AdminModal } from "@/components/admin/AdminModal";
import type { BestSellersHeadingsOnly } from "@/lib/cms/best-sellers-section-types";
import { CMS_LOCALES, type CmsLocale } from "@/lib/cms/hero-slider-types";

const FORM_ID = "best-sellers-headings-form";
...
type Props = {
  open: boolean;
  onClose: () => void;
  initial: BestSellersHeadingsOnly;
  onSaved: () => void;
};

export function BestSellersHeadingsForm({ open, onClose, initial, onSaved }: Props) {
  const t = useTranslations("adminUi");
  const tLocales = useTranslations("adminUi.cmsLocales");
  const [tab, setTab] = useState<CmsLocale>("en");
  const [draft, setDraft] = useState<BestSellersHeadingsOnly>(initial);
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      setSaving(true);
      setError(null);
      const res = await saveBestSellersSectionHeadings(draft);
      setSaving(false);
      if (!res.ok) {
  ...
  return (
    <AdminModal
      open={open}
      onClose={onClose}
      title={t("shared.headingsModalBestSellers")}
